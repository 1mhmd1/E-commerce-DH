...
export const deleteSkillByUser = async (
    ...
    try{
        ...
        return res.status(200).json({ message: `Skill '${name}' removed from user ${userPayload.username}` });
    }catch (err){
        next(err);
    }
}
...
export const addSkillGlobaly = async (
    ...
    try{
        ...
        const newSkillNames = skillNames.filter(name => !existingSkillsNames.includes(name));
