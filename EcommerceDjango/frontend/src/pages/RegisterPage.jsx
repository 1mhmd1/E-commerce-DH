...
export default function RegisterPage() {
  ...
  const handleRegister = async () => {
    const err = validate();
    if (err) return setStatus({ loading: false, error: err });
    ...
    try {
      ...
      if (detail) {
        if (detail.username) msg = `Username: ${detail.username[0]}`;
        else if (detail.email) msg = `Email: ${detail.email[0]}`;
        else if (detail.password) msg = `Password: ${detail.password[0]}`;
  ...
  const fieldStyle = {
    display: "block", fontSize: "0.75rem", fontWeight: 600, color: "#7A8499",
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "70vh", padding: "2rem 1rem" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "2rem" }}>
          ...
          <span style={{ fontWeight: 700, fontSize: "1.1rem", color: "#F0EBE1" }}>
        ...
        <p style={{ fontSize: "0.875rem", color: "#7A8499", marginBottom: "1.75rem" }}>
          Join DarkCart and start shopping.
        </p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
          <div>
            ...
            <input className="input-dark" placeholder="Choose a username" value={form.username}
              onChange={(e) => setForm((f) => ({ ...f, username: e.target.value }))} onKeyDown={handleKeyDown} autoComplete="username" />
          </div>

          <div>
            <label style={fieldStyle}>Email</label>
            <input className="input-dark" type="email" placeholder="you@example.com" value={form.email}
              onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} onKeyDown={handleKeyDown} autoComplete="email" />
          </div>

          <div>
            <label style={fieldStyle}>Password</label>
            <div style={{ position: "relative" }}>
              <input className="input-dark" type={showPwd ? "text" : "password"} placeholder="Min 8 characters"
                value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))}
                onKeyDown={handleKeyDown} autoComplete="new-password" style={{ paddingRight: "3rem" }} />
              <button type="button" onClick={() => setShowPwd((v) => !v)}
                style={{ position: "absolute", right: "0.875rem", top: "50%", transform: "translateY(-50%)",
                  background: "none", border: "none", color: "#4A5568", cursor: "pointer", display: "flex", padding: 0 }}>
          ...
          <div>
            ...
            <input className="input-dark" type={showPwd ? "text" : "password"} placeholder="Re-enter password"
        ...
        {status.error && (
          <div className="alert-error" style={{ marginTop: "1rem" }}>{status.error}</div>
        )}
        ...
        <button className="btn-primary" disabled={status.loading}
          ...
          {status.loading ? (
            <><div style={{ width: 16, height: 16, border: "2px solid #0F1117", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.6s linear infinite" }} /> Creating account…</>
          ) : (
        ...
        <p style={{ textAlign: "center", fontSize: "0.8125rem", color: "#4A5568", marginTop: "1.25rem" }}>
          ...
          <Link to="/login" style={{ color: "#4BB543", textDecoration: "none", fontWeight: 600 }}>Sign in</Link>
        </p>
      </motion.div>
    </div>
  );
